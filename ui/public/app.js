...
function filteredIssues(limit = 100) {
  ...
  return state.issues.filter((issue) =>
    ...
    && (!needle || `${issue.title} ${issue.level} ${issue.issue_type} ${issue.issue_category} ${issue.first_release} ${issue.last_release}`.toLowerCase().includes(needle)),
  ).slice(0, limit);
}
  ...
  return `
    ...
    ${rows.map((issue) => {
      ...
      return `<button class="table-row issue-grid issue-row-button" data-issue-id="${escapeHTML(issue.id)}">
        <div class="issue-name"><i class="severity ${level}"></i><div><strong>${issue.bookmarked ? '★ ' : ''}${escapeHTML(issue.title)}</strong><small><span class="status ${escapeHTML(issue.status)}">${escapeHTML(issue.status)}</span> ${escapeHTML(issue.priority || 'medium')} priority · ${escapeHTML((issue.issue_type || 'error').replaceAll('_', ' '))}${issue.assignee_name ? ` · ${escapeHTML(issue.assignee_name)}` : ''}</small></div></div>
...
function renderIssueDetail() {
  ...
  return `<div class="issue-detail-head"><button class="button secondary small" data-back-issues>← All issues</button><div class="inline-actions"><button class="icon-text-button ${issue.bookmarked ? 'active' : ''}" data-bookmark-issue>${issue.bookmarked ? '★ Bookmarked' : '☆ Bookmark'}</button><button class="button secondary small" data-snooze-issue>${issue.snoozed_until ? 'Unsnooze' : 'Snooze 24h'}</button><button class="button secondary small" data-issue-status="${issue.status === 'resolved' ? 'unresolved' : 'resolved'}">${issue.status === 'resolved' ? 'Reopen' : 'Resolve'}</button>${canAdminister() ? '<button class="button danger small" data-delete-issue>Delete</button>' : ''}</div></div>
    <section class="issue-hero card"><div><div class="issue-meta"><span class="severity ${escapeHTML(issue.level)}"></span><span class="status ${escapeHTML(issue.status)}">${escapeHTML(issue.status)}</span><span>${escapeHTML((issue.issue_type || 'error').replaceAll('_', ' '))}</span><span>${Number(issue.event_count).toLocaleString()} events</span>${issue.snoozed_until ? `<span>Snoozed until ${escapeHTML(new Date(issue.snoozed_until).toLocaleString())}</span>` : ''}</div><h2>${escapeHTML(issue.title)}</h2><p class="muted">First seen ${escapeHTML(relative(issue.first_seen_at))} · Last seen ${escapeHTML(relative(issue.last_seen_at))}</p></div><div class="issue-controls"><label>Assignee<select id="issue-assignee"><option value="">Unassigned</option><optgroup label="People">${state.members.members.map((member) => `<option value="user:${escapeHTML(member.id)}" ${issue.assignee_user_id === member.id ? 'selected' : ''}>${escapeHTML(member.name || member.email)}</option>`).join('')}</optgroup><optgroup label="Teams">${state.teams.filter((team) => team.projects?.some((project) => project.id === state.projectId)).map((team) => `<option value="team:${escapeHTML(team.id)}" ${issue.assignee_team_id === team.id ? 'selected' : ''}>#${escapeHTML(team.slug)}</option>`).join('')}</optgroup></select></label><label>Priority<select id="issue-priority">${['low', 'medium', 'high', 'critical'].map((value) => `<option value="${value}" ${issue.priority === value ? 'selected' : ''}>${value}</option>`).join('')}</select></label><label>Status<select id="issue-status">${['unresolved', 'resolved', 'ignored'].map((value) => `<option value="${value}" ${issue.status === value ? 'selected' : ''}>${value}</option>`).join('')}</select></label></div></section>
...
document.addEventListener('keydown', (event) => {
  if (event.key === 'Escape' && !desktopSidebar.matches) closeMobileSidebar();
});
desktopSidebar.addEventListener('change', syncSidebar);
$$('.primary-nav a').forEach((link) => { link.title = link.textContent.trim(); });
syncSidebar();
