...
| Native minidumps | Raw or multipart `/api/{project_id}/minidump/` ingestion, event metadata and attachments, bounded multi-thread x86/x86-64/ARM64 contexts, Breakpad `STACK CFI` unwinding, frame-pointer fallback, and debug-ID symbolication |
...
Native minidumps use bounded Breakpad `STACK CFI` rules or frame pointers to
unwind up to 256 captured threads and 2,048 total event frames. Windows
`STACK WIN`, DWARF `.eh_frame`, and architecture-specific compact-unwind
formats are not yet evaluated. PE/COFF
