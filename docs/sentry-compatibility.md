...
| Native minidumps | Raw or multipart `/api/{project_id}/minidump/` ingestion, event metadata and attachments, bounded multi-thread x86/x86-64/ARM64 contexts, Breakpad `STACK CFI` and Windows x86 `STACK WIN` unwinding, frame-pointer fallback, and debug-ID symbolication |
...
program/FPO metadata, or frame pointers to unwind up to 256 captured threads
and 2,048 total event frames. DWARF `.eh_frame` and architecture-specific
compact-unwind formats are not yet evaluated. PE/COFF
